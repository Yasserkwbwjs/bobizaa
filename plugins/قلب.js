const handler = async (m, { conn, command, text }) => {
  ...
  const response =
    ` 𓊆SU𝑵𝑮𓊇 ᗷOT࿐ هذا الامر لفتيات فقط `    
...
"انتهاء"
   ...
   let { key } = await conn.sendMessage(m.chat, {text: `*جاري تحميل*⌛`, mentions: conn.parseMention(response)}, {quoted: m})
